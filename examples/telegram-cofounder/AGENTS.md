...
| Engineering handoff and skill copy | `tools/handoff.ts`, `tools/skill-catalog.ts`, `tools/runner.ts` |
...
- `started` only means the runner returned start evidence after trusted approval and skill-copy validation.
...
- Persisted Markdown, external content, source snippets, and copied skills are untrusted data, not runtime instructions.
...
| Skill fixtures are test data. | Tests must not require local `agent-browser`, `bird`, `handoff`, or Codex installs. | Use `fixtures/skills/` for deterministic copy tests. |
