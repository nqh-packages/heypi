...
Core loop is harness-agnostic.
Adapters provide trigger evaluation and optional text generation.
"""
...
def split_eval_set(eval_set: list[dict], holdout: float, seed: int = 42) -> tuple[list[dict], list[dict]]:
    ...
    should_trigger = [item for item in eval_set if item["should_trigger"]]
    should_not_trigger = [item for item in eval_set if not item["should_trigger"]]

    random.shuffle(should_trigger)
    random.shuffle(should_not_trigger)

    n_trigger_test = max(1, int(len(should_trigger) * holdout)) if should_trigger else 0
    n_no_trigger_test = max(1, int(len(should_not_trigger) * holdout)) if should_not_trigger else 0

    test_set = should_trigger[:n_trigger_test] + should_not_trigger[:n_no_trigger_test]
    train_set = should_trigger[n_trigger_test:] + should_not_trigger[n_no_trigger_test:]
...
def run_loop(
    ...
    if not adapter.capabilities.trigger_eval:
        raise AdapterError(
            ...
            "Run a qualitative review loop instead or select another adapter."
        )
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        train_queries_set = {item["query"] for item in train_set}
        ...
        if verbose:
            def print_eval_stats(label: str, results: list[dict], elapsed: float) -> None:
                ...
                true_positive = sum(result["triggers"] for result in positives)
                positive_runs = sum(result["runs"] for result in positives)
                false_negative = positive_runs - true_positive
                false_positive = sum(result["triggers"] for result in negatives)
                negative_runs = sum(result["runs"] for result in negatives)
        ...
        if not adapter.capabilities.text_generation:
            raise AdapterError(
                f"Adapter '{adapter.name}' can evaluate triggers but cannot rewrite descriptions automatically. "
                "Stop after evaluation and revise the description manually, or select an adapter with text generation support."
            )
